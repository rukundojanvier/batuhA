...
if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) {
    $eid=@$_GET['eid'];
    // enforce payment/unlock rules before beginning question 1
    if($sn = @$_GET['n']){
        if($sn == 1) {
            // check if topic is free or already paid
            $free = false;
            $fr = mysqli_query($con,"SELECT eid FROM quiz ORDER BY date ASC LIMIT 1");
            if($fr && $frow = mysqli_fetch_array($fr)){
                if($frow['eid'] == $eid) $free = true;
            }
            if(!$free){
                $pj = mysqli_query($con,"SELECT * FROM payment WHERE email='$email' AND eid='$eid' AND status='paid'");
                if(mysqli_num_rows($pj) == 0){
                    header("location:account.php?q=needpayment&eid=$eid");
                    exit;
                }
            }
        }
